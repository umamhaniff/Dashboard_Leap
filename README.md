...
* **Dynamic Connection Failover (Module Hiding)**: Jika koneksi MariaDB lokal gagal dijangkau (misal saat di-deploy di Streamlit Cloud), sistem akan mendeteksi status offline secara dinamis. Halaman masuk (*login*) tetap menampilkan status offline dengan keterangan `No Local DB Connection`, dan setelah berhasil masuk, menu **Unified Overview** serta **Database SQL (Operations)** akan disembunyikan sepenuhnya dari navigasi dropdown sidebar. Dashboard otomatis diarahkan ke modul **Google Sheets: Absensi & Nilai** sebagai *landing page* default untuk menyajikan data operasional aktif tanpa membingungkan pengguna dengan data mock atau menu kosong.
